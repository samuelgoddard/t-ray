export default function Rollover({ label, underline, horizontal, animatedUnderline }) {
  return(
    <div className={`relative ${animatedUnderline ? 'py-[2px]' : 'py-[2px]'}`}>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-[1px] bg-current w-0 group-hover:w-full group-focus:w-full transition-all ease-in-out duration-300 opacity-0 ${animatedUnderline && 'opacity-100 delay-150'}`}></div>
    </div>
  )
}